...
int main()
{
    ...
    while(game)
    {
        ...
        bar(0,0,600,450);  //  initializing background size or wide (lower width 630 and right length 470)


            char x[20];
            sprintf(x," SCORE : %d",(food-1)*5);
          setcolor(RED);
         outtextxy(10,455, x);

          setcolor(RED);
         outtextxy(530,455,"Snake Game");



        setfillstyle(SOLID_FILL,RED);  // initializing boarder color

        bar(0,0,600,10); // upper boarder width,length

        bar(0,0,10,450); // right boarder width,length

        bar(0,450,600,440); // lower boarder width,length

        bar(590,10,600,450); // lower boarder width,length
    ...
    if(snakeX[0] >= 585|| snakeX[0] <=5 || snakeY[0] <= 5|| snakeY[0] >= 435){
    ...
    std::cout<<"Your score is : " <<(food-1)*5<<"\n\n";
    std::cout<<"Game Over !!!\n\n";
    getch();
    return 0;
}
